...
const getTimeFilter = (timeframe) => {
    ...
    switch (timeframe) {
        ...
        case 'last_7_days':
            ...
            break;
        case 'this_month':
            startTime = Math.floor(new Date(new Date().getFullYear(), new Date().getMonth(), 1).getTime() / 1000);
            break;
        case 'total':
        default:
            return '';
...
const getUserDataFromDb = async (userId) => {
    ...
    const userRow = await db.get('SELECT role FROM Users WHERE id = ?', [userId]);
    ...
    const devicesRows = await db.all('SELECT device_id FROM UserDevices WHERE user_id = ?', [userId]);
...
export const getMessageSummary = async (req, res) => {
    try {
        ...
        if (!userId) {
             return res.status(400).json({ error: "User ID wajib ada di query parameter." });
        }
        ...
        const whereClause = timeFilter ? `WHERE ${timeFilter} ${deviceFilter}` : (deviceFilter ? `WHERE 1=1 ${deviceFilter}` : '');
        ...
        return res.status(500).json({ error: err.message || 'Gagal mengambil ringkasan pesan.' });
    }
};
...
export const getDeviceSummary = async (req, res) => {
    try {
        ...
        const totalOnline = devices.filter(d => d.status.toLowerCase() === 'ready').length;
        ...
        return res.status(500).json({ error: err.message || 'Gagal mengambil ringkasan perangkat.' });
    }
};
...
export const getInboxSummary = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: err.message || 'Gagal mengambil ringkasan inbox.' });
    }
};
...
export const getRecentOutgoing = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: err.message || 'Gagal memuat pesan keluar terbaru.' });
    }
};
...
export const getRecentIncoming = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: err.message || 'Gagal memuat pesan masuk terbaru.' });
    }
};
...
export const getAdminMessageSummary = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: 'Gagal mengambil ringkasan pesan Admin.' });
    }
};
...
export const getAdminDeviceSummary = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: 'Gagal mengambil ringkasan perangkat Admin.' });
    }
};
...
export const getAdminInboxSummary = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: 'Gagal mengambil ringkasan inbox Admin.' });
    }
};
...
export const getAdminRecentOutgoing = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: 'Gagal memuat pesan keluar terbaru Admin.' });
    }
};
...
export const getAdminRecentIncoming = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: 'Gagal memuat pesan masuk terbaru Admin.' });
    }
};
// ==========================
...
export const getAdminScheduleDetails = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: 'Gagal memuat detail jadwal Admin.' });
    }
};
...
export const getScheduleDetails = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: err.message || 'Gagal memuat detail jadwal.' });
    }
};
